...
- **All CSS lives in `public/css/` files.** No `<style>` tags in templates/responses,
  no CSS-in-JS.
  - **Exception:** an inline `style` attribute is allowed *only* to pass a
...
- **Be as type-safe as possible.** Avoid `as any` casts — they hide bugs. Use proper
  types/interfaces; lean on Zod schemas for runtime validation *and* type inference.
...
- **Cover new functionality with tests** — happy paths *and* error cases.
