...
const score = { stars: 4.5, totalScore: 0.9, color: '#3bb54a', components: { coreMatch: 0.6, officialVideo: 0.3 } };
const spotify = (over = {}) => ({ id: 't1', name: 'Song Title', artist: 'The Artist', album: 'An Album', albumArt: img('cc3366'), ...over });
const youtube = (over = {}) => ({ id: 'vid1', title: 'Song Title (Official Video)', url: '#', thumbnail: img('3366cc'), ...over });

const detailsBase = { playlistId: 'pl1', playlistName: 'My Playlist', linkedCount: 1, totalTracks: 2, needsResync: false };

test('playlist-details: spotify-only (album art + text-only rows)', async ({ page }) => {
  await page.setContent(await pageFor('playlist-details.ejs', {
    ...detailsBase, hasYoutubeConnection: false, hasYoutubePlaylist: false,
    tracks: [
      { spotify: spotify(), youtube: null, linked: false },
      { spotify: spotify({ id: 't2', name: 'No Art Song', albumArt: '' }), youtube: null, linked: false }
    ]
  }));
...
test('playlist-details: youtube-connected-but-unsynced (album-art fix case)', async ({ page }) => {
  await page.setContent(await pageFor('playlist-details.ejs', {
    ...detailsBase, hasYoutubeConnection: true, hasYoutubePlaylist: false,
...
test('playlist-details: synced (linked video + unlinked placeholder + drift)', async ({ page }) => {
  await page.setContent(await pageFor('playlist-details.ejs', {
    ...detailsBase, hasYoutubeConnection: true, hasYoutubePlaylist: true, needsResync: true,
    tracks: [
      { spotify: spotify(), youtube: youtube(), linked: true, matchScore: score },
      { spotify: spotify({ id: 't2', name: 'Unlinked Song', albumArt: '' }), youtube: null, linked: false }
    ]
  }));
...
for (const [label, type, percentage, details] of [
  ...
  test(`progress-update: ${label}`, async ({ page }) => {
    await page.setContent(await pageFor('progress-update.ejs', { type, message: `State: ${label}`, percentage, details }));
    await expect(page.locator('.harness')).toHaveScreenshot(`progress-${label}.png`);
  });
}

test('error-message', async ({ page }) => {
  await page.setContent(await pageFor('error-message.ejs', {
    type: 'warning', title: 'YouTube Quota Exceeded', message: 'Your quota has been exceeded.', details: 'Resets at midnight Pacific.'
  }));
