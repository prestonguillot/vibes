...
import { YtPlaylist, YtPlaylistItem, YtPlaylistItemListResponse, YoutubeApiError } from '../utils/youtubeClient';
...
async function runSync(deps: SyncDeps): Promise<void> {
  ...
  if (tracks.length === 0) {
    ...
    emit(await renderPartial('sync-error.ejs', {
      playlistId,
  ...
  await emitProgress({
    type: 'progress',
    message: isUpdateMode ? `Updating existing playlist: "${playlistTitle}"` : 'Creating new YouTube playlist',
  ...
  if (isUpdateMode && unsyncedTracks.length === 0 && videoIds.length === 0) {
    ...
    const syncFeedbackHtml = await renderPartial('sync-feedback.ejs', {
      playlistId, isUpdate: true, isFullySynced: true,
      videosFound: 0, totalSearched: 0, isLimited: false, unlinkedTracks: []
    });
    ...
    emit(await renderPartial('sync-response.ejs', {
      playlistId, isUpdate: true,
      buttonText: 'Update YouTube Playlist', buttonClass: 'btn-outline-success',
      syncFeedbackHtml, playlistDetailsHtml,
      playlistName: playlist.name, trackCount: playlist.trackTotal ?? 0,
      spotifyUrl: playlist.spotifyUrl, youtubeUrl: youtubePlaylistUrl
    }));
    return;
  }
  ...
  if (!existingPlaylist) {
    const playlistResponse = await youtubeWrite('playlists.insert', () => youtube.playlists.insert({
      part: ['snippet', 'status'],
      requestBody: {
        snippet: { title: playlistTitle, description: `Synced from Spotify playlist: ${playlist.name}` },
    ...
    Logger.info('UPDATE MODE: Adding videos for next unsynced tracks', { videosFoundForNextTracks: videoIds.length });
    await emitProgress({
      type: 'progress',
  ...
  Logger.info('YouTube API Quota Usage Summary', {
    totalApiCalls: apiCallCount, totalQuotaUsed,
  ...
  const unlinkedTracks = searchResults.filter(r => !r.found).map(r => ({ track: r.track, artist: r.artist }));
  const syncFeedbackHtml = await renderPartial('sync-feedback.ejs', {
    playlistId, isUpdate: !!existingPlaylist,
    videosFound, totalSearched: searchResults.length,
    isLimited: tracks.length > trackLimit, totalTracks: tracks.length, unlinkedTracks
  });
  const playlistDetails = await fetchPlaylistDetails(spotifyAccessToken, youtube, playlistId, youtubePlaylistId);
  const playlistDetailsHtml = await renderPartial('playlist-details.ejs', {
    playlistId: playlistDetails.playlistId,
...
router.post('/playlist/:playlistId',
  ...
  async (req: ValidatedRequest<{ playlistId: string }, Record<string, unknown>, { batchSize?: string }>, res) => {
...
router.get('/playlist/:playlistId/stream',
  validate({
    params: z.object({ playlistId: schemas.spotifyPlaylistId }),
  ...
  async (req: ValidatedRequest<{ playlistId: string }, { batchSize?: string }>, res: Response) => {
    ...
    try {
      ...
      if (error instanceof Error && (error.message === 'SPOTIFY_AUTH_REQUIRED' || error.message === 'YOUTUBE_AUTH_REQUIRED')) {
        const service = error.message === 'SPOTIFY_AUTH_REQUIRED' ? 'Spotify' : 'YouTube';
        const loginUrl = error.message === 'SPOTIFY_AUTH_REQUIRED' ? '/auth/spotify/login' : '/auth/youtube/login';
        const html = await renderPartial('auth-expired.ejs', { service, loginUrl });
        return res.status(401).send(html);
      }
      Logger.error('Sync stream auth failed', { playlistId }, error);
      const html = await renderPartial('sync-error.ejs', {
        playlistId, title: 'Authentication Error',
    ...
    const emitProgress = async (update: ProgressUpdate) => {
      emit(await renderPartial('progress-update.ejs', {
