...
import { getYoutubeAuthUrl, exchangeYoutubeCode, createYoutubeClient, YoutubeApiError } from '../utils/youtubeClient';
import { z } from 'zod';
...
router.get('/callback',
  ...
  try {
    ...
    const channelsResponse = await youtube.channels.list({ part: ['id'], mine: true, maxResults: 1 });
    ...
    const serializedTokens = validateAndSerializeYouTubeTokens({ ...tokens, channel_id: channelId });
